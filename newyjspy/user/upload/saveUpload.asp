...
dim formsize,formdata,Msg,user_id
...
Msg = ""
		...
		if ftitle<>"" and fileSize > 0 and UploadSize=True then
			...
			dim basepath,sql
			basepath = "./uploadfile/"
			sql = "insert into INFO (filetitle,filedesc,filetype,filecontenttype,filepath,filesize,user_id) values ('"
			sql = sql & ftitle &"','"&filedesc&"','"&ftype&"','"&file_ctype&"','"&basepath&filename&"','"&filesize&"','"&user_id&"')"
			myconn.Execute(sql)
